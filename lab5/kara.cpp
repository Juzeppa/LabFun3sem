...
long_value karatsuba(long_value a, long_value b) {
  ...
  if (a.length < MIN_LENGTH_FOR_KARATSUBA) { //если число короче то применять наивное умножение
    printf("MAT' MERTVAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n");
    ...
    printf("apat1\n");
    for (int i = a_part1.length; i >0 ; i--)
      std::cout << a_part1.values[i - 1] << " ";
    printf("\n");
...
int main(int argc, char const *argv[])
{
  ...
  long_value chlen, bomja, chlen_bomja;
  chlen.values = a;
  bomja.values = b;
  chlen.length = bomja.length = 4;
  chlen_bomja = karatsuba(chlen, bomja);
  for (int i = chlen_bomja.length; i >0 ; i--)
    std::cout << chlen_bomja.values[i - 1] << " ";
