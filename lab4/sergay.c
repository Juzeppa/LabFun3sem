    ...
    while(!feof(fIn))
    {
       ...
       // printf("%lld\n", def_name);
        ...
        if ((res = FindReplace(HASH_TABLE[index], def_name)) != NULL)
            printf("%s ", FindReplace(HASH_TABLE[index], def_name));
        else printf("%s ", buf);
        bufptr = buf;
        while(!isspace(ch = fgetc(fIn)) && ch != EOF)
        {
            *bufptr++ = ch;
        }
        *bufptr = 0;
...
//    printf("\n\n\n");
//    for (size_t i = 0; i < HASHSIZE; i++) {
//        printDblLinkedList(HASH_TABLE[i]);
//        //printf("%ld", i);
//    }

//    list = createDblLinkedList();
//    pushFront(list, 14, "jija");
//    pushBack(list, 1488, "jopa");
